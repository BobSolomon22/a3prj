...
public class LastSkyscraperComponent extends GlassCockpitComponent {
    private static final int numDigitsShowing = 1;
    Image[] lastDigits = new Image[numDigitsShowing];

    public LastSkyscraperComponent() {
        super();

        for( int i = 0; i < numDigitsShowing; i++ )
            lastDigits[i] = digitImages[0];

        this.setLedColor(ColorUtil.rgb(255, 255, 0));
    }

    public void setLastSkyscraperReached( int lastSkyscraperReached ) {
        lastDigits[0] = digitImages[ lastSkyscraperReached % 10 ];
    }
